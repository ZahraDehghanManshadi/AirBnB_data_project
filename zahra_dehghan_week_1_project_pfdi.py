# -*- coding: utf-8 -*-
"""Zahra Dehghan - Week 1 Project - PFDI.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1uu2qUwo62baMelLHe0uNbBDRZB2l3CWe

**bold text**> 1. DUPLICATE THIS COLAB DOCUMENT TO START WORKING ON IT: On the top-left of this page, go to File > Save a copy to drive.
...
sek_rate = cc.convert(1, "USD", "SEK")
# Multiply the dollar column by your currency of choice
matrix[:, 1] = sek_rate*matrix[:, 1]
...
inflation_rate = 0.1 # 10% of inflation in Sweden
matrix[:,1] *=(1 + 0.1)
...
def from_location_to_airbnb_listing_in_meters1(lat1: float, lon1: float, lat2: list, lon2: list):
...
for i, j in zip(matrix[:, 2], matrix[:, 3]):
    vector = from_location_to_airbnb_listing_in_meters1(latitude, longitude, i, j)
...
# conv_to_meters = np.vectorize(from_location_to_airbnb_listing_in_meters1)
# 
...
def from_location_to_airbnb_listing_in_meters(lat1: float, lon1: float, lat2: list, lon2: list):   
    R = 6371000  # Radius of Earth in meters
    phi_1 = np.deg2rad(lat1) # CHANGE THIS
    phi_2 = np.deg2rad(lat2) # CHANGE THIS

    delta_phi = np.deg2rad(lat2 - lat1) # CHANGE THIS
    delta_lambda = np.deg2rad(lon2 - lon1) # CHANGE THIS
...
for i, j in zip(matrix[:, 2], matrix[:, 3]):
    vector = from_location_to_airbnb_listing_in_meters(latitude, longitude, i, j)
...
matrix2 = np.concatenate((matrix, colors), axis=1)
matrix2[:3, 4:]

# Append our entry to the matrix
fav_entry = np.array([1, 0, 52.3584, 4.8811, 0, 1]).reshape(1, -1) # Change coordinates to your favorite location

matrix2 = np.concatenate((fav_entry, matrix2), axis=0)
...
# # Round of values
# dataframe["Price"] = "SEK " + dataframe["Price"].round(2).astype(str) # <--- CHANGE THIS POUND SYMBOL IF YOU CHOSE CURRENCY OTHER THAN POUND
